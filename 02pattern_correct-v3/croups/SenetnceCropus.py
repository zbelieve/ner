...
class Sentence():
    def __init__(self, sentence=None):
        ...
        self.e1_List = Dan
        # 这儿将弹丸质量都替换成了NumG,得到segSentenceNumG
        qualityList = []
        for word in self.e2_List:
            qualityList.append(word[0])
        segSentenceNumG = []
        k = 0
        for word in self.segSentence2:
            if word in qualityList:
                segSentenceNumG.append('NumG')
            else:
                segSentenceNumG.append(word)
            k=k+1
        self.segSentenceNumG = segSentenceNumG
        self.vec1 = None
        self.vec2 = None
        self.bef = None
        self.bet = None
    ...
    def regSub2(self,strs):
        file = open("../spo/subject.txt", encoding='UTF-8')
        Dict = []
    ...
    # 识别弹丸,输入是主语识别后的主语列表
    def regDan(self,sub):
        file = open("../spo/subjectDan.txt", encoding='UTF-8')
        DictDan = []
        for line in file.readlines():
            lineN = line.split("\n")[0]
